...
if not price_data:
    ...
    with tab3:
        ...
        # 提取显示列并格式化
        display_cols = ['trade_date', 'close_price', 'high_price', 'low_price', 
                       'volume', 'daily_volatility', 'weekly_volatility']
        
        if 'ma20' in df_display.columns:
            display_cols.append('ma20')
        
        df_display_cols = df_display[[col for col in display_cols if col in df_display.columns]].copy()
        
        # 重命名列
        df_display_cols.columns = [
            col.replace('trade_date', '交易日期')
            .replace('close_price', '收盘价')
            .replace('high_price', '最高价')
            .replace('low_price', '最低价')
            .replace('volume', '成交量')
            .replace('daily_volatility', '日波动率')
            .replace('weekly_volatility', '周波动率')
            .replace('ma20', '20日MA')
            for col in df_display_cols.columns
        ]
        
        st.dataframe(df_display_cols, use_container_width=True, height=600)
